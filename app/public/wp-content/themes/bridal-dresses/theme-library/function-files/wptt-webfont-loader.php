...
if ( ! class_exists( 'Bridal_Dresses_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Bridal_Dresses_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $bridal_dresses_url The remote URL.
		 */
		public function __construct( $bridal_dresses_url = '' ) {
			$this->remote_url = $bridal_dresses_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $bridal_dresses_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $bridal_dresses_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $bridal_dresses_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $bridal_dresses_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $bridal_dresses_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $bridal_dresses_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $bridal_dresses_format = 'woff2' ) {
			$this->font_format = $bridal_dresses_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $bridal_dresses_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $bridal_dresses_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $bridal_dresses_url, '/' ) ) {
				$bridal_dresses_parsed_url = parse_url( $this->remote_url );
				return $bridal_dresses_parsed_url['scheme'] . '://' . $bridal_dresses_parsed_url['hostname'] . $bridal_dresses_url;
			}

			return $bridal_dresses_url;
		}
	}
}

if ( ! function_exists( 'bridal_dresses_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 *
	 * @param string $bridal_dresses_url    The URL of the remote webfont.
	 * @param string $bridal_dresses_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function bridal_dresses_wptt_get_webfont_styles( $bridal_dresses_url, $bridal_dresses_format = 'woff2' ) {
		$bridal_dresses_font = new Bridal_Dresses_WPTT_WebFont_Loader( $bridal_dresses_url );
		$bridal_dresses_font->set_font_format( $bridal_dresses_format );
		return $bridal_dresses_font->get_styles();
	}
}

if ( ! function_exists( 'bridal_dresses_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function bridal_dresses_wptt_get_webfont_url( $bridal_dresses_url, $bridal_dresses_format = 'woff2' ) {
		$bridal_dresses_font = new Bridal_Dresses_WPTT_WebFont_Loader( $bridal_dresses_url );
		$bridal_dresses_font->set_font_format( $bridal_dresses_format );
		return $bridal_dresses_font->get_url();
	}
}