...
class PHPM_Admin_Settings {
    ...
    public function register_settings() {
        ...
        add_settings_section(
            ...
            array($this, 'render_frontend_pages_section'),
            'phpm_settings'
        );
        
        // Property listing page
        add_settings_field(
            'phpm_property_listing_page',
            __('Property Listings Page', 'plughaus-property'),
            array($this, 'render_property_listing_page_field'),
            'phpm_settings',
            'phpm_frontend_pages'
        );
        ...
        // Tenant portal page
        add_settings_field(
            'phpm_tenant_portal_page',
            __('Tenant Portal Page', 'plughaus-property'),
            array($this, 'render_tenant_portal_page_field'),
            'phpm_settings',
            'phpm_frontend_pages'
        );
        ...
        add_settings_field(
            'phpm_maintenance_request_page',
            __('Maintenance Request Page', 'plughaus-property'),
            array($this, 'render_maintenance_request_page_field'),
            'phpm_settings',
            'phpm_frontend_pages'
        );
    }
    ...
    public function sanitize_settings($input) {
        ...
        // Sanitize frontend page settings
        if (isset($input['property_listing_page'])) {
            $sanitized['property_listing_page'] = absint($input['property_listing_page']);
        }
        if (isset($input['tenant_portal_page'])) {
            $sanitized['tenant_portal_page'] = absint($input['tenant_portal_page']);
        }
        if (isset($input['maintenance_request_page'])) {
            $sanitized['maintenance_request_page'] = absint($input['maintenance_request_page']);
        }
    ...
    /**
     * Render frontend pages section
     */
    public function render_frontend_pages_section() {
        echo '<p>' . __('Assign WordPress pages to display property management content. Create pages with the appropriate shortcodes to enable frontend functionality.', 'plughaus-property') . '</p>';
    }
    
    /**
     * Render property listing page field
     */
    public function render_property_listing_page_field() {
        $options = get_option('phpm_settings');
        $selected_page = isset($options['property_listing_page']) ? $options['property_listing_page'] : 0;
        
        $pages = get_pages(array('post_status' => 'publish,private,draft'));
        
        echo '<select name="phpm_settings[property_listing_page]" id="phpm_property_listing_page">';
        echo '<option value="0">' . __('— Select Page —', 'plughaus-property') . '</option>';
        foreach ($pages as $page) {
            echo '<option value="' . esc_attr($page->ID) . '"' . selected($selected_page, $page->ID, false) . '>';
            echo esc_html($page->post_title);
            echo '</option>';
        }
        echo '</select>';
        echo '<p class="description">' . __('Page to display property listings. Use shortcode: [phpm_property_listings]', 'plughaus-property') . '</p>';
    }
    
    /**
     * Render tenant portal page field
     */
    public function render_tenant_portal_page_field() {
        $options = get_option('phpm_settings');
        $selected_page = isset($options['tenant_portal_page']) ? $options['tenant_portal_page'] : 0;
        
        $pages = get_pages(array('post_status' => 'publish,private,draft'));
        
        echo '<select name="phpm_settings[tenant_portal_page]" id="phpm_tenant_portal_page">';
        echo '<option value="0">' . __('— Select Page —', 'plughaus-property') . '</option>';
        foreach ($pages as $page) {
            echo '<option value="' . esc_attr($page->ID) . '"' . selected($selected_page, $page->ID, false) . '>';
            echo esc_html($page->post_title);
        ...
        echo '</select>';
        echo '<p class="description">' . __('Page for tenant portal access. Use shortcode: [phpm_tenant_portal]', 'plughaus-property') . '</p>';
    }
    
    /**
     * Render maintenance request page field
     */
    public function render_maintenance_request_page_field() {
        $options = get_option('phpm_settings');
        $selected_page = isset($options['maintenance_request_page']) ? $options['maintenance_request_page'] : 0;
        
        $pages = get_pages(array('post_status' => 'publish,private,draft'));
        
        echo '<select name="phpm_settings[maintenance_request_page]" id="phpm_maintenance_request_page">';
        echo '<option value="0">' . __('— Select Page —', 'plughaus-property') . '</option>';
        foreach ($pages as $page) {
            echo '<option value="' . esc_attr($page->ID) . '"' . selected($selected_page, $page->ID, false) . '>';
            echo esc_html($page->post_title);
        ...
        echo '</select>';
        echo '<p class="description">' . __('Page for maintenance request form. Use shortcode: [phpm_maintenance_request_form]', 'plughaus-property') . '</p>';
    }
}
